...
class SudokuViewModelV2: ObservableObject {
    ...
    init(sudoku: Sudoku) {
        ...
        cells.forEach{ (row) in
            row.forEach{ (cell) in
                var top = false; var leading = false; var bottom = false; var trailing = false;
                // top
                if cell.row() > 0 {
                ...
                print("b:\(cell.box()),\(cell.row()),\(cell.col())", cell.boxBorder)
            }
        }
...
class CellViewModel: ObservableObject,Hashable {
    ...
    func setBoxBorder(top: Bool = false, leading: Bool = false, bottom: Bool = false, trailing: Bool = false) {
        if top {
